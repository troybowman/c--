...
public:
  parser_ctx_t() : mode(CTX_GLOBAL), _syms(&gsyms) {}
  ~parser_ctx_t() { clear(); }
  ...
  void setglobal()          { clear(); mode = CTX_GLOBAL; _syms = &gsyms; }
  void setlocal(symref_t f) { clear(); mode = CTX_LOCAL;  _syms = f->symbols(); putref(_func, f); }
  void settemp()            { clear(); mode = CTX_TEMP;   _syms = new symtab_t; }

  symref_t &func() const { return getref(_func); }
  void insert(symref_t sym) { _syms->insert(sym); }
